 ...
 * 通过XML构建Mybatis中的Configuration对象
 */
public class XMLConfigBuilder extends BaseBuilder {
    ...
    private Properties settingsAsProperties(XNode context) {
        ...
        /** 将setting中的name和value解析成properties */
        Properties props = context.getChildrenAsProperties();
        // Check that all settings are known to the configuration class
        // 检查配置类是否知道所有设置
        MetaClass metaConfig = MetaClass.forClass(Configuration.class, localReflectorFactory);
        for (Object key : props.keySet()) {
            /** 检查的方式通过判断通过反射判断这个字段是否有setter方法 */
    ...
    /** 将所有的setting解析后的properties保存到配置对象中,mybatis所有支持的设置都在这 */
    ...
    private TransactionFactory transactionManagerElement(XNode context) throws Exception {
        if (context != null) {
            /** 获取事务管理类型, mybatis提供了两种方式管理事务. */
            String type = context.getStringAttribute("type");
            /** 如果存在transactionManager节点下存在properties,获取事务管理属性. */
            Properties props = context.getChildrenAsProperties();
            // todo how to resolve class by JDBC?
    ...
    private DataSourceFactory dataSourceElement(XNode context) throws Exception {
        if (context != null) {
            String type = context.getStringAttribute("type");
            Properties props = context.getChildrenAsProperties();
            // todo how to resolve class by jdbc or unpooled?
     ...
     *     <package name="com.zsj.mybatis.mapper"/>
     * </mappers>
     */
